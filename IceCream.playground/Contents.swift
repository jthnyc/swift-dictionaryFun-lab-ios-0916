...
func namesForIceCreamFlavor(flavor: String) -> [String] {
    var namesForEach: [String] = [] // create a new variable to append the names to
    
    for (names, flavors) in favoriteFlavorsOfIceCream {
        if flavors == flavor {
            namesForEach.append(names)
        }
    }
...
print(namesForIceCreamFlavor("Natural Vanilla"))
...
//    to change a flavor: call on the function
//    let newTim = favoriteFlavorsOfIceCream["Tim"] = "Pistachio"
    
    let keys = favoriteFlavorsOfIceCream.keys
    
    if !keys.contains(person) {
        return false

    } else if flavor != favoriteFlavorsOfIceCream[person] {
...
func addPerson(name: String, withFlavor flavor: String) -> Bool {
    ...
    // if a new person was added, update what their favorite flavor is and return true
    // if a person already exists, don't update their flavor and return false
